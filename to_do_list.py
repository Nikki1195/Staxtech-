...
#create a empty list 
task=[]
# create to add function 
def add():
    global task
    title = entry.get()  # get input
   #use strip to avoid blank tasks
    if title.strip():
        task.append({"title": title.strip(), "completed": False})
        entry.delete(0, tr.END)
        update()
    else:
        messagebox.showwarning("empty task", "Enter a valid task")
...
def comtask():
    ...
    if select is not None and 0 <= select < len(task):
        ...
        messagebox.showerror("Selected is error ", "Please select a task first.")
...
def dele() :
    ...
    if select is not None and 0 <= select < len(task):
        ...
        messagebox.showerror("Selection is Error", "Please select a task first.")
...
tr.Label(
         root,
         text="Click a task to mark or delete",
         font=("Arial",10),
         bg="#87CEFA",
         fg="white"
).pack(pady=10)
